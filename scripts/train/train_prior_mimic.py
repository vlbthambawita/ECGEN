#!/usr/bin/env python3
"""
Train PixelCNN Prior (Stage 2) on MIMIC-IV-ECG dataset using PyTorch Lightning.
...
from models.vqvae import PriorLightning, PriorConfig
...
class PriorMIMICDataModule(pl.LightningDataModule):
    """Data module for Prior training with MIMIC-IV-ECG"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train PixelCNN Prior for VQ-VAE (Stage 2)")
    
    # Config file option
    parser.add_argument("--config", type=str, default=None, help="Path to YAML config file")
    
    # Experiment
    parser.add_argument("--exp-name", type=str, default="prior_mimic", help="Experiment name")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--runs-root", type=str, default="runs", help="Root directory for runs")
    
    # VQ-VAE checkpoint (required)
    parser.add_argument(
        "--vqvae-checkpoint",
        type=str,
        required=True,
        help="Path to trained VQ-VAE checkpoint from Stage 1",
    )
    ...
    # Model
    parser.add_argument("--num-embeddings", type=int, default=512, help="Codebook size (must match VQ-VAE)")
    ...
    parser.add_argument("--wandb-project", type=str, default="ecg-vqvae-prior", help="W&B project name")
    ...
    parser.add_argument("--wandb-tags", type=str, nargs="*", default=["vqvae", "prior", "pixelcnn"], help="W&B tags")
...
def main() -> None:
    ...
    if not args.config and not args.data_dir:
        ...
        print("Usage:")
        print("  python train_prior_mimic.py --config configs/experiments/prior_mimic.yaml --vqvae-checkpoint path/to/vqvae.ckpt")
        print("  python train_prior_mimic.py --data-dir /path/to/mimic-iv-ecg --vqvae-checkpoint path/to/vqvae.ckpt")
        sys.exit(1)
    ...
    if args.config:
        ...
        if 'model' in yaml_config and 'params' in yaml_config['model']:
            ...
            args.num_embeddings = model_cfg.get('num_embeddings', args.num_embeddings)
            args.hidden_dim = model_cfg.get('hidden_dim', args.hidden_dim)
            args.num_layers = model_cfg.get('num_layers', args.num_layers)
            args.lr = model_cfg.get('lr', args.lr)
            args.b1 = model_cfg.get('b1', args.b1)
            args.b2 = model_cfg.get('b2', args.b2)
            # VQ-VAE checkpoint can be overridden by config
            if 'vqvae_checkpoint' in model_cfg:
                args.vqvae_checkpoint = model_cfg['vqvae_checkpoint']
    ...
    datamodule = PriorMIMICDataModule(
    ...
    # Train
    print("=" * 80)
    print(f"Starting Prior training (Stage 2) for experiment '{args.exp_name}'")
    print("=" * 80)
