#!/usr/bin/env python3
"""
Train VQ-VAE (Stage 1) on MIMIC-IV-ECG dataset using PyTorch Lightning.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train VQ-VAE for ECG generation (Stage 1)")
    ...
    parser.add_argument("--wandb-tags", type=str, nargs="*", default=["vqvae", "ecg"], help="W&B tags")
...
def main() -> None:
    ...
    if not args.config and not args.data_dir:
        ...
        print("Usage:")
        print("  python train_vqvae_mimic.py --config configs/experiments/vqvae_mimic.yaml")
        print("  python train_vqvae_mimic.py --data-dir /path/to/mimic-iv-ecg")
        sys.exit(1)
    ...
    if args.config:
        ...
        if 'model' in yaml_config and 'params' in yaml_config['model']:
            ...
            args.commitment_cost = model_cfg.get('commitment_cost', args.commitment_cost)
    ...
    # Train
    print("=" * 80)
    print(f"Starting VQ-VAE training (Stage 1) for experiment '{args.exp_name}'")
    print("=" * 80)
    ...
    print(f"  python scripts/train/train_prior_mimic.py --vqvae-checkpoint {checkpoint_callback.best_model_path}")
    print("=" * 80)
